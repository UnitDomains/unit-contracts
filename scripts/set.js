...
async function main() {
  ...
  const EnsRegistry = await (
    await hre.ethers.getContractFactory("ENSRegistry")
  ).attach(registryAddress);
  const BaseRegistrar = await (
    await hre.ethers.getContractFactory("BaseRegistrarImplementation")
  ).attach(registrarAddress);
  const controller = await (
    await hre.ethers.getContractFactory("ETHRegistrarController")
